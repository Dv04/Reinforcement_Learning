...
class SoftQNetwork(nn.Module):
    def __init__(
        self, num_inputs, num_actions, hidden_size, activation=F.relu, init_w=3e-3
    ):
...
class PolicyNetwork(nn.Module):
    ...
    def get_action(self, state, deterministic):
        ...
        action = (
            torch.tanh(mean).detach().cpu().numpy()[0]
            if deterministic
...
def update(batch_size, reward_scale, gamma=0.99, soft_tau=1e-2):
    ...
    reward = (
        reward_scale * (reward - reward.mean(dim=0)) / reward.std(dim=0)
    ...
    target_value_func = (
        predicted_new_q_value - alpha * log_prob
    )  # for stochastic training, it equals to expectation over action
...
if ENV == "Reacher":
    ...
    env = Reacher(
        ...
        ini_joint_angles=INI_JOING_ANGLES,
        target_pos=[369, 430],
        render=True,
        change_goal=False,
    )
...
soft_q_net1 = SoftQNetwork(state_dim, action_dim, hidden_dim, activation=F.relu).to(
    device
)
soft_q_net2 = SoftQNetwork(state_dim, action_dim, hidden_dim, activation=F.relu).to(
    device
)
policy_net = PolicyNetwork(state_dim, action_dim, hidden_dim, activation=F.relu).to(
    device
)
...
max_episodes = 1000
max_steps = (
    20 if ENV == "Reacher" else 150
)  # Pendulum needs 150 steps per episode to learn well, cannot handle 20
frame_idx = 0
batch_size = 128
explore_steps = 0
rewards = []
